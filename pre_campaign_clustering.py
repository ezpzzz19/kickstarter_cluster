...
def resample_to_realistic_distribution(df, target_success_rate=0.30, target_failure_rate=0.61, random_state=42):
    """
    ...
    print(f"Target: {target_success_rate*100:.0f}% success, {target_failure_rate*100:.0f}% failure")
    
    # Filter out 'live' projects
    df_filtered = df[df['state'] != 'live'].copy()
    
    # Get current distribution
    current_dist = df_filtered['state'].value_counts(normalize=True)
    ...
    target_failure_count = int(total_projects * target_failure_rate)
    target_other_count = total_projects - target_success_count - target_failure_count
    
    print(f"Target counts: {target_success_count} success, {target_failure_count} failure, {target_other_count} other")
    
    # Get current counts
    current_success = df_filtered[df_filtered['state'] == 'successful']
    current_failure = df_filtered[df_filtered['state'] == 'failed']
    current_other = df_filtered[df_filtered['state'] == 'canceled']
    ...
    if len(current_success) > target_success_count:
        ...
        success_sample = current_success.sample(n=target_success_count, random_state=random_state)
    else:
        # Upsample successful projects with replacement
        success_sample = current_success.sample(n=target_success_count, replace=True, random_state=random_state)
    ...
    if len(current_failure) > target_failure_count:
        ...
        failure_sample = current_failure.sample(n=target_failure_count, random_state=random_state)
    else:
        # Upsample failed projects with replacement
        failure_sample = current_failure.sample(n=target_failure_count, replace=True, random_state=random_state)
    
    # For other projects (canceled): downsample if too many, upsample if too few
    if len(current_other) > target_other_count:
        # Downsample other projects
        other_sample = current_other.sample(n=target_other_count, random_state=random_state)
    else:
        # Upsample other projects with replacement
        other_sample = current_other.sample(n=target_other_count, replace=True, random_state=random_state)
    
    # Combine resampled data
    resampled_df = pd.concat([success_sample, failure_sample, other_sample], ignore_index=True)
    
    # Shuffle the data
    resampled_df = resampled_df.sample(frac=1, random_state=random_state).reset_index(drop=True)
...
def analyze_pre_campaign_features():
    ...
    df = resample_to_realistic_distribution(df, target_success_rate=0.30, target_failure_rate=0.61)
    ...
    print(f"\nStaff pick analysis:")
    print(f"staff_pick True: {df['staff_pick'].sum()} ({df['staff_pick'].mean()*100:.1f}%)")
    print(f"staff_pick.1 True: {df['staff_pick.1'].sum()} ({df['staff_pick.1'].mean()*100:.1f}%)")
    print(f"Coalesced True: {df['staff_pick_coalesced'].sum()} ({df['staff_pick_coalesced'].mean()*100:.1f}%)")
    
    # Create goal tiers (useful for clustering)
    df['goal_tier'] = pd.cut(df['goal_usd'], 
                           bins=[0, 1000, 5000, 10000, 50000, 100000, np.inf],
                           labels=['Micro', 'Small', 'Medium', 'Large', 'Very Large', 'Mega'])
    
    # Create duration tiers
    df['duration_tier'] = pd.cut(df['project_duration_days'],
    ...
    clustering_df['goal_usd'] = clustering_df['goal_usd'].fillna(clustering_df['goal_usd'].median())
    clustering_df['name_len_clean'] = clustering_df['name_len_clean'].fillna(clustering_df['name_len_clean'].median())
    clustering_df['blurb_len_clean'] = clustering_df['blurb_len_clean'].fillna(clustering_df['blurb_len_clean'].median())
    clustering_df['project_duration_days'] = clustering_df['project_duration_days'].fillna(clustering_df['project_duration_days'].median())
    
    # Boolean features should be False if missing
    clustering_df['staff_pick_coalesced'] = clustering_df['staff_pick_coalesced'].fillna(False)
    clustering_df['show_feature_image'] = clustering_df['show_feature_image'].fillna(False)
    ...
    for k in k_values:
        ...
        print(f"k={k:2d}: Inertia={inertia:10.2f}, Silhouette={sil_score:6.3f}, Calinski={cal_score:8.1f}")
    ...
    print(f"Using k={optimal_k} for final clustering (optimal based on silhouette score)")
    ...
    final_kmeans = KMeans(n_clusters=optimal_k, n_init=20, max_iter=500, random_state=42)
    ...
    success_analysis = df.groupby('cluster')['state'].value_counts().unstack(fill_value=0)
    success_analysis['total'] = success_analysis.sum(axis=1)
    success_analysis['success_rate'] = (success_analysis['successful'] / success_analysis['total'] * 100).round(2)
    success_analysis['failure_rate'] = (success_analysis['failed'] / success_analysis['total'] * 100).round(2)
    ...
    for cluster in range(optimal_k):
        ...
        cluster_summary[f'cluster_{cluster}'] = {
            ...
            'canceled_count': len(cluster_data[cluster_data['state'] == 'canceled']),
            'success_rate': len(cluster_data[cluster_data['state'] == 'successful']) / len(cluster_data) * 100,
            'failure_rate': len(cluster_data[cluster_data['state'] == 'failed']) / len(cluster_data) * 100,
            ...
            'pct_micro_goal': len(cluster_data[cluster_data['goal_tier'] == 'Micro']) / len(cluster_data) * 100,
            'pct_small_goal': len(cluster_data[cluster_data['goal_tier'] == 'Small']) / len(cluster_data) * 100,
            'pct_medium_goal': len(cluster_data[cluster_data['goal_tier'] == 'Medium']) / len(cluster_data) * 100,
            'pct_large_goal': len(cluster_data[cluster_data['goal_tier'] == 'Large']) / len(cluster_data) * 100,
            'pct_very_large_goal': len(cluster_data[cluster_data['goal_tier'] == 'Very Large']) / len(cluster_data) * 100,
            'pct_mega_goal': len(cluster_data[cluster_data['goal_tier'] == 'Mega']) / len(cluster_data) * 100,
            
            # Duration tiers distribution
            'pct_very_short_duration': len(cluster_data[cluster_data['duration_tier'] == 'Very Short']) / len(cluster_data) * 100,
            'pct_short_duration': len(cluster_data[cluster_data['duration_tier'] == 'Short']) / len(cluster_data) * 100,
            'pct_medium_duration': len(cluster_data[cluster_data['duration_tier'] == 'Medium']) / len(cluster_data) * 100,
            'pct_long_duration': len(cluster_data[cluster_data['duration_tier'] == 'Long']) / len(cluster_data) * 100,
            'pct_very_long_duration': len(cluster_data[cluster_data['duration_tier'] == 'Very Long']) / len(cluster_data) * 100,
            
            # Season distribution
            'pct_winter_launch': len(cluster_data[cluster_data['launch_season'] == 'winter']) / len(cluster_data) * 100,
            'pct_spring_launch': len(cluster_data[cluster_data['launch_season'] == 'spring']) / len(cluster_data) * 100,
            'pct_summer_launch': len(cluster_data[cluster_data['launch_season'] == 'summer']) / len(cluster_data) * 100,
            'pct_fall_launch': len(cluster_data[cluster_data['launch_season'] == 'fall']) / len(cluster_data) * 100,
            
            # Continent distribution
            'pct_north_america': len(cluster_data[cluster_data['continent'] == 'North America']) / len(cluster_data) * 100,
            'pct_europe': len(cluster_data[cluster_data['continent'] == 'Europe']) / len(cluster_data) * 100,
            'pct_asia': len(cluster_data[cluster_data['continent'] == 'Asia']) / len(cluster_data) * 100,
            'pct_oceania': len(cluster_data[cluster_data['continent'] == 'Oceania']) / len(cluster_data) * 100,
            'pct_south_america': len(cluster_data[cluster_data['continent'] == 'South America']) / len(cluster_data) * 100,
            'pct_africa': len(cluster_data[cluster_data['continent'] == 'Africa']) / len(cluster_data) * 100,
        }
    ...
    print("\nComprehensive Cluster Summary:")
    print(cluster_summary_df[['cluster_id', 'size', 'success_rate', 'failure_rate', 
                             'avg_goal_usd', 'avg_duration_days', 'pct_staff_pick', 
                             'pct_has_video', 'pct_north_america', 'pct_europe']])
    ...
    print("✓ Avoided post-campaign data leakage (no pledged, backers_count, spotlight, etc.)")
    ...
    print(f"Worst performing cluster: Cluster {success_analysis['success_rate'].idxmin()} "
